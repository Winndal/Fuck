<!--<!DOCTYPE html>
<html>
	<head>
		<title>UNICS</title>
		<link rel="stylesheet" type="text/css" href="Assets/css/main.css">
		<meta charset="UTF-8">
	</head>
	<body>
		<img src="Assets/img/lov.jpg">
		<div id="div_wrapper">
			<a id="" >Welcome to University Crowdsourcing!</a>
		</div>
		<div id="main">
			<div id="div_userbox" class="bigbox">
				<div id="div_info">
				<label id="infoP">UniCs - A social crowdsourcing platform which enables students to search, add courses, upload and download course material.</label> 
				<img src="Assets/img/icon.png" id="logga">
					<nav class="full">
						<ul class="menu">
							<li><h1><a href="index.php">Start</a></h1></li>
							<li><h1><a href="sign_up.php">Sign up</a></h1></li>
							<li><h1><a href="info.php" class="active">Info</a></h1></li>
							<li><h1><a href="donate.php">Donate</a></h1></li>
						</ul>
					</nav>
				</div>
			</div>
		</div>
	</body>
</html>